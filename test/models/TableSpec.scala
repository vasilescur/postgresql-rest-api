...
class TableSpec extends Specification{
  ...
  "Table.getTableContentByName" should {
    ...
    "return some table content" in new App {
      ...
      val result = dao.DAO.getTableContentByName(dao.QueryBuilder.select("books")) map {
        case a: JsValue  => {
          a mustEqual json
          success
        }
    ...
    "return None if there is no result" in new App {
        val result = dao.DAO.getTableContentByName(dao.QueryBuilder.select("catalogs")) map {
